...
lifecycle.on('sessionExpired', async ({ reason }) => {
  consoleLog('Session expired:', reason);
  // Handle expiration
});

lifecycle.on('sessionResumed', ({ session }) => {
  consoleLog('Session resumed:', session);
  // Restore UI state
});
```
...
class SessionAnalytics {
  ...
  trackSessionEnd(session: any) {
    ...
    this.metrics.averageDuration =
      (this.metrics.averageDuration * (this.metrics.totalSessions - 1) +
        duration) /
      this.metrics.totalSessions;
...
class SessionTestSuite {
  ...
  private async testSessionCreation() {
    consoleLog('Testing session creation...');
  ...
  private async testSessionExpiry() {
    consoleLog('Testing session expiry...');
