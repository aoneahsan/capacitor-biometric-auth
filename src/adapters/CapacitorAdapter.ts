...
export class CapacitorAdapter implements BiometricAuthAdapter {
  ...
  private async getPlugin() {
    ...
    try {
      ...
      if ((capacitorCore as any).Plugins?.BiometricAuth) {
        this.capacitorPlugin = (capacitorCore as any).Plugins.BiometricAuth;
        return this.capacitorPlugin;
      }

      // If not found in Plugins, try direct import
      // This allows the plugin to work even if not properly registered
      const BiometricAuthPlugin = (window as any).BiometricAuthPlugin;
  ...
  async getSupportedBiometrics(): Promise<BiometryType[]> {
    try {
      ...
      return (result.biometryTypes || []).map((type: string) => {
    ...
    if (error?.code) {
      switch (error.code) {
        ...
        case 'UNAVAILABLE':
          ...
          message = error.message || 'Biometric authentication is not available';
          break;
        case 'USER_CANCELLED':
        case 'CANCELLED':
        case 'USER_CANCEL':
          code = BiometricErrorCode.USER_CANCELLED;
          message = error.message || 'User cancelled authentication';
          break;
        ...
        case 'TIMEOUT':
          ...
          message = error.message || 'Authentication timed out';
          break;
        case 'LOCKOUT':
        ...
        case 'NOT_ENROLLED':
          ...
          message = error.message || 'No biometric credentials enrolled';
          break;
        default:
